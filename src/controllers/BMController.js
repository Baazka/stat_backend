...
WHERE SA.ID = :P_ID`;
const FindIDs = `SELECT SA.ID, DEPARTMENT_ID, FP.PERIOD_ID, SA.AUDIT_TYPE_ID 
FROM AUD_STAT.STAT_AUDIT SA 
...
module.exports = {
  async BM1List(req, res) {
    try {
      ...
      //params.P_AUDIT_TYPE_ID = resultFindID.rows[0]?.AUDIT_TYPE_ID;
      ...
      NVL(L.CONCLUSION, FAS.CONCLUSION) CONCLUSION,
      NVL(L.CONCLUSION_DESC, FAS.CONCLUSION_DESC) CONCLUSION_DESC,
      FAS.HOSLUULAH HOSLUULAH,
      ...
      NVL(L.TUL_BENEFIT_TOO, FAS.TUL_BENEFIT_TOO) TUL_BENEFIT_TOO,
      NVL(L.TOD_BENEFIT, FAS.TOD_BENEFIT) TOD_BENEFIT,
      NVL(L.TOD_BENEFIT_TOO, FAS.TOD_BENEFIT_TOO) TOD_BENEFIT_TOO,
      NVL(L.TOD_BUS_BENEFIT, FAS.TOD_BUS_BENEFIT) TOD_BUS_BENEFIT,
      NVL(L.TOD_BUS_BENEFIT_TOO, FAS.TOD_BUS_BENEFIT_TOO) TOD_BUS_BENEFIT_TOO,
      ...
      NVL(L.AUDIT_PERCENT, FAS.PERCENT_CNT) AUDIT_PERCENT
      ...
      RAT.AUDIT_TYPE_ID,
      RAT.AUDIT_TYPE_NAME,
      CASE WHEN RAT.AUDIT_TYPE_ID = 2 THEN 'Тийм' ELSE 'Үгүй' END HOSLUULAH,
      NVL(EN.PERIOD5_NAME, NVL(EN.PERIOD4_NAME, NVL(EN.PERIOD3_NAME, AE.ENT_NAME))) ||' - '|| RAT.AUDIT_TYPE_NAME ||' - '|| RP.YEAR_LABEL AUDIT_NAME,
      FA.AUDIT_CODE,
      ...
      (NVL(CASE WHEN(CASE WHEN FA.STATUS = 1 AND FA.PERIOD_ID > 3 THEN 3 ELSE FA.AUDIT_FORM_TYPE END) = 1 THEN A.SUO_TOO ELSE B.SUO_TOO END, 0) + 
      NVL(CASE WHEN(CASE WHEN FA.STATUS = 1 AND FA.PERIOD_ID > 3 THEN 3 ELSE FA.AUDIT_FORM_TYPE END) = 1 THEN D.ZAL_TOO ELSE D1.ZAL_SAMPLE_TOO END, 0 )) TOD_BENEFIT_TOO, 
      (NVL(CASE WHEN(CASE WHEN FA.STATUS = 1 AND FA.PERIOD_ID > 3 THEN 3 ELSE FA.AUDIT_FORM_TYPE END) = 1 THEN A.SBUO ELSE B.SBUO END, 0) + 
      NVL(CASE WHEN(CASE WHEN FA.STATUS = 1 AND FA.PERIOD_ID > 3 THEN 3 ELSE FA.AUDIT_FORM_TYPE END) = 1 THEN D.ZAL_AMOUNT ELSE D1.ZAL_AMOUNT_SAMPLE END, 0 )) TOD_BUS_BENEFIT, 
      (NVL(CASE WHEN(CASE WHEN FA.STATUS = 1 AND FA.PERIOD_ID > 3 THEN 3 ELSE FA.AUDIT_FORM_TYPE END) = 1 THEN A.SBUO_TOO ELSE B.SBUO_TOO END, 0) + 
      NVL(CASE WHEN(CASE WHEN FA.STATUS = 1 AND FA.PERIOD_ID > 3 THEN 3 ELSE FA.AUDIT_FORM_TYPE END) = 1 THEN D.ZAL_TOO ELSE D1.ZAL_SAMPLE_TOO END, 0 )) TOD_BUS_BENEFIT_TOO, 
      FA.AUDIT_ORG_TYPE,
      ...
      DECODE((CASE WHEN FA.STATUS = 1 AND FA.PERIOD_ID > 3 THEN 3 ELSE FA.AUDIT_FORM_TYPE END), 2, NULL, CND.CONCLUSION_DESC) CONCLUSION_DESC,
      FP.PERC PERCENT_CNT
      ...
      LEFT JOIN FAS_ADMIN.REF_PERIOD RP ON FA.PERIOD_ID = RP.PERIOD_ID
      LEFT JOIN FAS_ADMIN.FAS_DOCUMENT_DATA FDD2 ON FA.ID = FDD2.FAS_AUDIT_ID AND FDD2.IS_ACTIVE = 1 AND FDD2.IND_ID IN (71,346,437,534) AND FDD2.DOCUMENT_ID = (SELECT DOCUMENT_ID FROM FAS_ADMIN.REF_DOCUMENT_LIST_FORM
        WHERE FORM_TYPE_ID = (CASE WHEN FA.STATUS = 1 AND ((FA.PERIOD_ID = 4 AND FA.AUDIT_FORM_TYPE IN (1,2)) OR (FA.PERIOD_ID = 5 AND FA.AUDIT_FORM_TYPE = 1)) THEN 3 
        WHEN FA.STATUS = 1 AND FA.PERIOD_ID = 5 AND FA.AUDIT_FORM_TYPE = 2 THEN 4 
        ELSE FA.AUDIT_FORM_TYPE END) AND DOCUMENT_ID IN (103,106,7,33)
        AND PERIOD_ID = FA.PERIOD_ID)
      ...
      INNER JOIN (SELECT FA.ID, ROUND( COUNT(DISTINCT(DP.DOCUMENT_ID)) / COUNT(DISTINCT(LF.DOCUMENT_ID)) * 100,2) PERC 
      FROM FAS_ADMIN.FAS_AUDIT FA
      INNER JOIN FAS_ADMIN.REF_DOCUMENT_LIST_FORM LF ON FA.PERIOD_ID = LF.PERIOD_ID AND (CASE WHEN FA.STATUS = 1 AND FA.PERIOD_ID > 3 THEN 3 ELSE FA.AUDIT_FORM_TYPE END) = LF.FORM_TYPE_ID AND LF.IS_PERCENTAGE = 1
      LEFT JOIN FAS_ADMIN.FAS_DOC_PROCESS DP ON DP.DOCUMENT_ID = LF.DOCUMENT_ID AND DP.FAS_AUDIT_ID = FA.ID
      GROUP BY FA.ID) FP ON FA.ID = FP.ID
      ...
      LEFT JOIN (SELECT FAS_AUDIT_ID, IND_VALUE AS CONCLUSION_DESC
      FROM FAS_ADMIN.FAS_DOCUMENT_DATA  
      WHERE DOCUMENT_ID IN (26, 132) AND IND_ID IN (723, 340) AND IND_VALUE IS NOT NULL) CND ON FA.ID = CND.FAS_AUDIT_ID
      --START UR UGUUJ
      ...
      LEFT JOIN (SELECT A.FAS_AUDIT_ID, SUM(CASE WHEN A.UR_UGUUJ_TYPE = 320 THEN A.AMOUNT END) SUO, COUNT(CASE WHEN A.UR_UGUUJ_TYPE = 320 THEN A.AMOUNT END) SUO_TOO,
         SUM(CASE WHEN A.UR_UGUUJ_TYPE = 321 THEN A.AMOUNT END) SBUO, COUNT(CASE WHEN A.UR_UGUUJ_TYPE = 321 THEN A.AMOUNT END) SBUO_TOO 
      FROM 
      ...
      LEFT JOIN (SELECT A.FAS_AUDIT_ID, SUM(A.AMOUNT) ZAL_AMOUNT, COUNT(A.AMOUNT) ZAL_TOO
        FROM FAS_ADMIN.FAS_DOC_5_1 A
      ...
      LEFT JOIN (SELECT A.FAS_AUDIT_ID, SUM(ZAL_AMOUNT_SAMPLE) ZAL_AMOUNT_SAMPLE, COUNT(ZAL_AMOUNT_SAMPLE) ZAL_SAMPLE_TOO FROM
  ...
  async BM1IU(req, res) {
    try {
      const queryBM1 = `BEGIN AUD_STAT.NEW_BM1_I_U(:P_ID, :P_STAT_AUDIT_ID, :P_AUDIT_ID, :P_AUDIT_TYPE_ID, :P_IS_EXPERT_ATTEND, :P_IS_PRESS_REPORT, :P_WORK_PEOPLE, :P_WORK_DAY, :P_WORK_TIME, :P_LOG_ID, :P_AUDIT_TYPE_NAME, :P_AUDIT_NAME, :P_AUDIT_CODE, :P_ENT_NAME, :P_ORG_REGISTER_NO, :P_BUDGET_TYPE_ID, :P_BUDGET_SHORT_NAME, :P_SALBAR_ANGILAL, :P_FORM_TYPE_ID, :P_AUDIT_TYPE, :P_AUDIT_ORG_TYPE, :P_AUDIT_ORG_CHECK_NAME, :P_DEPARTMENT_NAME, :P_CHECK_DEPARTMENT_NAME, :P_AUDIT_DEPARTMENT_NAME, :P_AUDIT_LEAD, :P_AUDIT_MEMBER, :P_REASON_TYPE_ID, :P_REASON_TYPE_NAME, :P_CONCLUSION, :P_CONCLUSION_DESC, :P_TUL_BENEFIT_TOO, :P_TUL_BENEFIT, :P_TOD_BENEFIT_TOO, :P_TOD_BENEFIT, :P_TOD_BUS_BENEFIT_TOO, :P_TOD_BUS_BENEFIT, :P_AUDIT_PERCENT, :P_CREATED_BY); END;`;
        ...
        if (req.body.data?.length > 0) {
          req.body.data?.forEach((element) => {
            data.push({
              ...
              P_CONCLUSION_DESC: element.CONCLUSION_DESC,
              P_TUL_BENEFIT_TOO: CheckNullInt(element.TUL_BENEFIT_TOO),
              P_TUL_BENEFIT: CheckNullFloat(element.TUL_BENEFIT),
              P_TOD_BENEFIT_TOO: CheckNullInt(element.TOD_BENEFIT_TOO),
              P_TOD_BENEFIT: CheckNullFloat(element.TOD_BENEFIT),
              P_TOD_BUS_BENEFIT_TOO: CheckNullInt(element.TOD_BUS_BENEFIT_TOO),
              P_TOD_BUS_BENEFIT: CheckNullFloat(element.TOD_BUS_BENEFIT),
              P_AUDIT_PERCENT: CheckNullFloat(element.AUDIT_PERCENT),
  ...
  async BM2List(req, res) {
    try {
     ...
     NVL(L.AUDIT_DEPARTMENT_NAME, FAS.AUDIT_DEPARTMENT_NAME) AUDIT_DEPARTMENT_NAME,
      ...
      RBT.BUDGET_TYPE_ID,
      RBT.BUDGET_SHORT_NAME,
      CASE WHEN FA.STATUS = 1 THEN 'Төрийн удирдлага, батлан хамгаалах үйл ажиллагаа, албан журмын нийгмийн хамгаалал' ELSE RDI.IND_VALUE_NAME END SALBAR_ANGILAL,
      (CASE WHEN RCD.DEPARTMENT_NAME IS NOT NULL AND FA.AUDIT_ORG_TYPE = 1 AND FA.AUDIT_CHECK_DEP_ID IN (101,102) THEN RD.DEPARTMENT_NAME||' - '||RCD.DEPARTMENT_NAME ELSE RD.DEPARTMENT_NAME END) DEPARTMENT_NAME,
      RD.DEPARTMENT_NAME AS CHECK_DEPARTMENT_NAME,
      RD1.DEPARTMENT_NAME AS AUDIT_DEPARTMENT_NAME,
      'Шалгагдагч этгээд' ORG_TYPE
  ...
  async BM2IU(req, res) {
    try {
      const queryBM2 = `BEGIN AUD_STAT.NEW_BM2_I_U(:P_ID, :P_STAT_AUDIT_ID, :P_AUDIT_ID, :P_AUDIT_TYPE_ID, :P_LOG_ID, :P_AUDIT_TYPE_NAME, :P_AUDIT_NAME, :P_AUDIT_CODE, :P_FORM_TYPE_ID, :P_AUDIT_TYPE, :P_AUDIT_ORG_TYPE, :P_AUDIT_ORG_CHECK_NAME, :P_ENT_NAME, :P_ORG_REGISTER_NO, :P_BUDGET_TYPE_ID, :P_BUDGET_SHORT_NAME, :P_SALBAR_ANGILAL, :P_DEPARTMENT_NAME, :P_CHECK_DEPARTMENT_NAME, :P_AUDIT_DEPARTMENT_NAME, :P_CREATED_BY); END;`;
  ...
  async BM3List(req, res) {
    try {
      ...
      let ListQuery = `SELECT 
        ...
        NVL(L.CHECK_DEPARTMENT_NAME, FAS.CHECK_DEPARTMENT_NAME) CHECK_DEPARTMENT_NAME,
        NVL(L.AUDIT_DEPARTMENT_NAME, FAS.AUDIT_DEPARTMENT_NAME) AUDIT_DEPARTMENT_NAME,
        NVL(L.IS_ZALRUULAH, FAS.IS_ZALRUULAH) IS_ZALRUULAH,
        NVL(L.IS_ZALRUULAH_NAME, FAS.IS_ZALRUULAH_NAME) IS_ZALRUULAH_NAME,
        NVL(L.ALD_SHORT_DESC, FAS.ALD_SHORT_DESC) ALD_SHORT_DESC,
        NVL(L.AMOUNT, FAS.AMOUNT) AMOUNT,
        NVL(L.IS_SUB_ERROR_CONFLICT, FAS.IS_SUB_ERROR_CONFLICT) IS_SUB_ERROR_CONFLICT,
        NVL(L.IS_SUB_ERROR_CONFLICT_NAME, FAS.IS_SUB_ERROR_CONFLICT_NAME) IS_SUB_ERROR_CONFLICT_NAME,
        NVL(L.UR_UGUUJ, FAS.UR_UGUUJ) UR_UGUUJ,
        NVL(L.UR_UGUUJ_NAME, FAS.UR_UGUUJ_NAME) UR_UGUUJ_NAME,
        NVL(L.UR_UGUUJ_TYPE, FAS.UR_UGUUJ_TYPE) UR_UGUUJ_TYPE,
        NVL(L.UR_UGUUJ_TYPE_NAME, FAS.UR_UGUUJ_TYPE_NAME) UR_UGUUJ_TYPE_NAME,
        NVL(L.SALBAR_ANGILAL, FAS.SALBAR_ANGILAL) SALBAR_ANGILAL        
      ...
      SELECT 
        ...
        INNER JOIN (
          ...
          A.UR_UGUUJ, V1.IND_VALUE_NAME UR_UGUUJ_NAME, B.UR_UGUUJ_TYPE, V2.IND_VALUE_NAME UR_UGUUJ_TYPE_NAME, A.IS_SUB_ERROR_CONFLICT, V3.IND_VALUE_NAME IS_SUB_ERROR_CONFLICT_NAME, 1 TABLE_TYPE_ID  -- 1-T52, 2-T55, 3-TH, 4-D55, 5-D52
          FROM FAS_ADMIN.FAS_DOC_5_2_SAMPLE A
          ...
          A.UR_UGUUJ, V1.IND_VALUE_NAME UR_UGUUJ_NAME, B.UR_UGUUJ_TYPE, V2.IND_VALUE_NAME UR_UGUUJ_TYPE_NAME, A.IS_SUB_ERROR_CONFLICT, V3.IND_VALUE_NAME IS_SUB_ERROR_CONFLICT_NAME, 2 TABLE_TYPE_ID  -- 1-T52, 2-T55, 3-TH, 4-D55, 5-D52
          FROM FAS_ADMIN.FAS_DOC_5_5_SAMPLE A
          LEFT JOIN FAS_ADMIN.FAS_DOC_5_6_SAMPLE B ON A.ID = B.ID_5_5 AND B.IS_CLAUSE = 0
          LEFT JOIN FAS_ADMIN.REF_DOCUMENT_INDICATOR_VALUE V1 ON A.UR_UGUUJ = V1.ID
          LEFT JOIN FAS_ADMIN.REF_DOCUMENT_INDICATOR_VALUE V2 ON A.UR_UGUUJ_TYPE = V2.ID
          LEFT JOIN FAS_ADMIN.REF_DOCUMENT_INDICATOR_VALUE V3 ON A.IS_SUB_ERROR_CONFLICT = V3.ID
          WHERE A.IS_ACTIVE = 1 AND A.IS_ERROR_CONFLICT = 285
          UNION
          --TH
          ...
          A.UR_UGUUJ_TYPE, 
          V2.IND_VALUE_NAME UR_UGUUJ_TYPE_NAME, 
          A.IS_SUB_ERROR_CONFLICT, V3.IND_VALUE_NAME IS_SUB_ERROR_CONFLICT_NAME, 3 TABLE_TYPE_ID  -- 1-T52, 2-T55, 3-TH, 4-D55, 5-D52
          FROM FAS_ADMIN.FAS_DOC_5_1_CLAUSE A
          ...
          B.UR_UGUUJ_TYPE, V2.IND_VALUE_NAME UR_UGUUJ_TYPE_NAME, 
          NULL, NULL, 4 TABLE_TYPE_ID  -- 1-T52, 2-T55, 3-TH, 4-D55, 5-D52
          FROM FAS_ADMIN.FAS_DOC_5_1 A
          INNER JOIN FAS_ADMIN.FAS_DOC_5_5 B ON A.ID = B.ID_5_1
          ...
          WHERE A.IS_ACTIVE = 1 AND A.IS_ERROR_CONFLICT = 285 AND A.IS_ZALRUULAH = 284
          UNION
          --DUGNELT 52
          SELECT A.FAS_AUDIT_ID, A.ID, A.SHORT_DESC, A.AMOUNT, A.IS_ZALRUULAH, V5.IND_VALUE_NAME IS_ZALRUULAH_NAME, 
          B.UR_UGUUJ, V1.IND_VALUE_NAME UR_UGUUJ_NAME, 
          320 UR_UGUUJ_TYPE, 'Санхүүгийн' UR_UGUUJ_TYPE_NAME,
          B.IS_SUB_ERROR_CONFLICT, V3.IND_VALUE_NAME IS_SUB_ERROR_CONFLICT_NAME, 5 TABLE_TYPE_ID  -- 1-T52, 2-T55, 3-TH, 4-D55, 5-D52
          FROM FAS_ADMIN.FAS_DOC_5_1 A
          INNER JOIN FAS_ADMIN.FAS_DOC_5_2 B ON A.ID = B.ID_5_1
          LEFT JOIN FAS_ADMIN.FAS_DOC_5_6 D ON A.ID = D.ID_5_1 AND D.IS_CLAUSE = 0
          LEFT JOIN FAS_ADMIN.REF_DOCUMENT_INDICATOR_VALUE V1 ON B.UR_UGUUJ = V1.ID
  ...
  async BM3IU(req, res) {
    try {
      const queryBM3 = `BEGIN AUD_STAT.NEW_BM3_I_U(:P_ID, :P_STAT_AUDIT_ID, :P_AUDIT_ID, :P_ERROR_ID, :P_TABLE_TYPE_ID, :P_AUDIT_TYPE_ID, :P_LOG_ID, :P_AUDIT_TYPE_NAME, :P_AUDIT_NAME, :P_AUDIT_CODE, :P_FORM_TYPE_ID, :P_AUDIT_TYPE, :P_AUDIT_ORG_TYPE, :P_AUDIT_ORG_CHECK_NAME, :P_ENT_NAME, :P_ORG_REGISTER_NO, :P_BUDGET_TYPE_ID, :P_BUDGET_SHORT_NAME, :P_CHECK_DEPARTMENT_NAME, :P_AUDIT_DEPARTMENT_NAME, :P_IS_ZALRUULAH, :P_IS_ZALRUULAH_NAME, :P_ALD_SHORT_DESC, :P_AMOUNT, :P_IS_SUB_ERROR_CONFLICT, :P_IS_SUB_ERROR_CONFLICT_NAME, :P_UR_UGUUJ, :P_UR_UGUUJ_NAME, :P_UR_UGUUJ_TYPE, :P_UR_UGUUJ_TYPE_NAME, :P_SALBAR_ANGILAL, :P_CREATED_BY); END;`;
        ...
        if (req.body.data?.length > 0) {
          req.body.data?.forEach((element) => {
            data.push({
              ...
              P_CHECK_DEPARTMENT_NAME: element.CHECK_DEPARTMENT_NAME,
              P_AUDIT_DEPARTMENT_NAME: element.AUDIT_DEPARTMENT_NAME,
              P_ERROR_ID: CheckNullInt(element.ERROR_ID),
              P_TABLE_TYPE_ID: CheckNullInt(element.TABLE_TYPE_ID),
  ...
  async BM4List(req, res) {
    try {
      ...
      FAS.SOLUTION_NAME,
      FAS.AMOUNT,
      FAS.IS_ERROR_CONFLICT,
      FAS.IS_ERROR_CONFLICT_NAME,

      FAS.UR_UGUUJ,
      FAS.UR_UGUUJ_NAME,
      FAS.UR_UGUUJ_TYPE,
      FAS.UR_UGUUJ_TYPE_NAME,
      FAS.AKT_DATE,
      FAS.AKT_NO,
      FAS.SHORT_DESC,
      FAS.COMPLETION_DATE,
      ...
      FAS.MO_DATE,
      FAS.PROCESS_AMOUNT,
      FAS.PROCESS_EVALUATION,
      FAS.ACCOUNT_TYPE,
      FAS.ACCOUNT_TYPE_NAME,
      FAS.FULL_NAME,
      FAS.NEXT_AMOUNT,
      FAS.SALBAR_ANGILAL    
--        NVL(L.AUDIT_TYPE_NAME, FAS.AUDIT_TYPE_NAME) AUDIT_TYPE_NAME,
...
--        NVL(L.CHECK_DEPARTMENT_NAME, FAS.CHECK_DEPARTMENT_NAME) CHECK_DEPARTMENT_NAME,
--        NVL(L.AUDIT_DEPARTMENT_NAME, FAS.AUDIT_DEPARTMENT_NAME) AUDIT_DEPARTMENT_NAME,
--        NVL(L.ID_7_1, FAS.ID_7_1) ID_7_1,
--        NVL(L.ALD_SHORT_DESC, FAS.ALD_SHORT_DESC) ALD_SHORT_DESC,
--        NVL(L.SOLUTION, FAS.SOLUTION) SOLUTION,
--        NVL(L.SOLUTION_NAME, FAS.SOLUTION_NAME) SOLUTION_NAME,
--        NVL(L.AMOUNT, FAS.AMOUNT) AMOUNT,
--        NVL(L.IS_ERROR_CONFLICT, FAS.IS_ERROR_CONFLICT) IS_ERROR_CONFLICT,
--        NVL(L.IS_ERROR_CONFLICT_NAME, FAS.IS_ERROR_CONFLICT_NAME) IS_ERROR_CONFLICT_NAME,
--
--        NVL(L.UR_UGUUJ, FAS.UR_UGUUJ) UR_UGUUJ,
--        NVL(L.UR_UGUUJ_NAME, FAS.UR_UGUUJ_NAME) UR_UGUUJ_NAME,
--        NVL(L.UR_UGUUJ_TYPE, FAS.UR_UGUUJ_TYPE) UR_UGUUJ_TYPE,
--        NVL(L.UR_UGUUJ_TYPE_NAME, FAS.UR_UGUUJ_TYPE_NAME) UR_UGUUJ_TYPE_NAME,
--        NVL(L.AKT_DATE, FAS.AKT_DATE) AKT_DATE,
--        NVL(L.AKT_NO, FAS.AKT_NO) AKT_NO,
--        NVL(L.SHORT_DESC, FAS.SHORT_DESC) SHORT_DESC,
--        NVL(L.COMPLETION_DATE, FAS.COMPLETION_DATE) COMPLETION_DATE,
--        NVL(L.USER_ID, FAS.USER_ID) USER_ID,
--        NVL(L.AUDITOR_NAME, FAS.AUDITOR_NAME) AUDITOR_NAME,
--        NVL(L.AUDITOR_CODE, FAS.AUDITOR_CODE) AUDITOR_CODE,
--        NVL(L.MO_DATE, FAS.MO_DATE) MO_DATE,
--        NVL(L.MO_AMOUNT, FAS.PROCESS_AMOUNT) PROCESS_AMOUNT,
--        NVL(L.MO_AMOUNT, FAS.PROCESS_EVALUATION) PROCESS_EVALUATION,
--        NVL(L.ACCOUNT_TYPE, FAS.ACCOUNT_TYPE) ACCOUNT_TYPE,
--        NVL(L.ACCOUNT_TYPE_NAME, FAS.ACCOUNT_TYPE_NAME) ACCOUNT_TYPE_NAME,
--        NVL(L.FULL_NAME, FAS.FULL_NAME) FULL_NAME,
--        NVL(L.NEXT_AMOUNT, FAS.NEXT_AMOUNT) NEXT_AMOUNT,
--        NVL(L.SALBAR_ANGILAL, FAS.SALBAR_ANGILAL) SALBAR_ANGILAL        
      ...
      A.ID ID_7_1, A.ALD_SHORT_DESC,
        ...
        A.IS_SUB_ERROR_CONFLICT, A.IS_SUB_ERROR_CONFLICT_NAME,
        A.UR_UGUUJ, A.UR_UGUUJ_NAME,
        A.UR_UGUUJ_TYPE, A.UR_UGUUJ_TYPE_NAME,

        A.COMPLETION_DATE,
        A.USER_ID, A.AUDITOR_NAME, A.AUDITOR_CODE, A.MO_DATE, A.PROCESS_AMOUNT, A.PROCESS_EVALUATION,
        A.ACCOUNT_TYPE, A.ACCOUNT_TYPE_NAME, A.FULL_NAME, 
        BM1L.SALBAR_ANGILAL,
        CASE WHEN LENGTH(DD.IND_VALUE) = 10 THEN TO_DATE(DD.IND_VALUE,'YYYY-MM-DD') ELSE NULL END AKT_DATE, A.AKT_NO, A.SHORT_DESC,
        A.AMOUNT - A.PROCESS_AMOUNT NEXT_AMOUNT
      FROM FAS_ADMIN.FAS_AUDIT FA
      INNER JOIN AUD_ORG.AUDIT_ENTITY AE ON FA.ENT_ID = AE.ENT_ID
      INNER JOIN AUD_ORG.AUDIT_ORGANIZATION AO ON AE.ENT_ORG_ID = AO.ORG_ID
      LEFT JOIN FAS_ADMIN.ENTITY_NAME EN ON FA.ENT_ID = EN.ENT_ID
      ...
      LEFT JOIN AUD_STAT.NEW_BM1_LOG BM1L ON FA.ID = BM1L.AUDIT_ID AND BM1L.AUDIT_TYPE_ID = 2
      LEFT JOIN FAS_ADMIN.FAS_DOCUMENT_DATA DD ON FA.ID = DD.FAS_AUDIT_ID AND DD.DOCUMENT_ID = 133 AND DD.IND_ID = 762
      INNER JOIN (SELECT 
A.FAS_AUDIT_ID, A.ID, A.AMOUNT, A.SOLUTION, A.SOLUTION_NAME, A.SHORT_DESC, A.COMPLETION_DATE, 
A.ALD_SHORT_DESC, A.AKT_NO, A.FULL_NAME,
A.IS_ERROR_CONFLICT, V1.IND_VALUE_NAME IS_ERROR_CONFLICT_NAME,
A.IS_SUB_ERROR_CONFLICT, V2.IND_VALUE_NAME IS_SUB_ERROR_CONFLICT_NAME,
A.UR_UGUUJ, V3.IND_VALUE_NAME UR_UGUUJ_NAME,
A.UR_UGUUJ_TYPE, V4.IND_VALUE_NAME UR_UGUUJ_TYPE_NAME,
A.ACCOUNT_TYPE, V5.IND_VALUE_NAME ACCOUNT_TYPE_NAME,
SU.USER_ID, SU.USER_NAME AUDITOR_NAME, SU.USER_CODE AUDITOR_CODE, M.MO_DATE,         
CASE WHEN SYSDATE <= A.COMPLETION_DATE THEN 1 ELSE 0 END IS_DATE,
  AE.PROCESS_EVALUATION,
  PROCESS_AMOUNT
  FROM (SELECT C.ID, NULL ID_5_1, A.ID ID_5_1_CLAUSE, A.AMOUNT, C.COMPLETION_DATE,
  A.SOLUTION, V1.IND_VALUE_NAME SOLUTION_NAME,
  A.IS_ERROR_CONFLICT, A.IS_SUB_ERROR_CONFLICT, A.UR_UGUUJ, A.UR_UGUUJ_TYPE, C.ACCOUNT_TYPE, C.AKT_NO, SUBSTR(C.LASTNAME,1,1)||'.'||C.FIRSTNAME FULL_NAME,
  C.SHORT_DESC, A.FAS_AUDIT_ID, A.SHORT_DESC ALD_SHORT_DESC, 3 TABLE_TYPE --3 TTX, 2 DUGNELT, 1 TUUWER
  FROM FAS_ADMIN.FAS_DOC_5_1_CLAUSE A
  INNER JOIN FAS_ADMIN.REF_DOCUMENT_INDICATOR_VALUE V ON A.IS_MATERIAL = V.ID
  ...
  UNION 
  SELECT C.ID, A.ID ID_5_1, NULL ID_5_1_CLAUSE, A.AMOUNT, C.COMPLETION_DATE, 
  B.SOLUTION, V1.IND_VALUE_NAME SOLUTION_NAME, 
  A.IS_ERROR_CONFLICT, NULL, B.UR_UGUUJ, B.UR_UGUUJ_TYPE, C.ACCOUNT_TYPE, C.AKT_NO, SUBSTR(C.LASTNAME,1,1)||'.'||C.FIRSTNAME FULL_NAME,
  C.SHORT_DESC, A.FAS_AUDIT_ID, A.SHORT_DESC ALD_SHORT_DESC, 2
  FROM FAS_ADMIN.FAS_DOC_5_1 A
  INNER JOIN FAS_ADMIN.REF_DOCUMENT_INDICATOR_VALUE V ON A.IS_MATERIAL = V.ID
  INNER JOIN FAS_ADMIN.FAS_DOC_5_5 B ON A.ID = B.ID_5_1
  ...
  WHERE A.IS_ACTIVE = 1 AND A.IS_ZALRUULAH = 284 AND B.SOLUTION != 341 AND A.IS_ERROR_CONFLICT IS NOT NULL 
  UNION
  SELECT C.ID, A.ID ID_5_5, NULL ID_5_1_CLAUSE, A.AMOUNT, C.COMPLETION_DATE, 
  A.SOLUTION, V1.IND_VALUE_NAME SOLUTION_NAME,
  A.IS_ERROR_CONFLICT, A.IS_SUB_ERROR_CONFLICT, A.UR_UGUUJ, A.UR_UGUUJ_TYPE, C.ACCOUNT_TYPE, C.AKT_NO, SUBSTR(C.LASTNAME,1,1)||'.'||C.FIRSTNAME FULL_NAME,
  C.SHORT_DESC, A.FAS_AUDIT_ID, A.SHORT_DESC ALD_SHORT_DESC, 1
  FROM FAS_ADMIN.FAS_DOC_5_5_SAMPLE A
  INNER JOIN FAS_ADMIN.FAS_DOC_7_1 C ON A.ID = C.ID_5_5
  LEFT JOIN FAS_ADMIN.REF_DOCUMENT_INDICATOR_VALUE V1 ON A.SOLUTION = V1.ID
  WHERE A.IS_ACTIVE = 1 AND A.SOLUTION != 341 ) A
LEFT JOIN FAS_ADMIN.REF_DOCUMENT_INDICATOR_VALUE V1 ON A.IS_ERROR_CONFLICT = V1.ID
LEFT JOIN FAS_ADMIN.REF_DOCUMENT_INDICATOR_VALUE V2 ON A.IS_SUB_ERROR_CONFLICT = V2.ID
LEFT JOIN FAS_ADMIN.REF_DOCUMENT_INDICATOR_VALUE V3 ON A.UR_UGUUJ = V3.ID
LEFT JOIN FAS_ADMIN.REF_DOCUMENT_INDICATOR_VALUE V4 ON A.UR_UGUUJ_TYPE = V4.ID
LEFT JOIN FAS_ADMIN.REF_DOCUMENT_INDICATOR_VALUE V5 ON A.ACCOUNT_TYPE = V5.ID
LEFT JOIN FAS_ADMIN.AKT_MONITORING M ON A.ID = M.SEVEN_ONE_ID AND M.IS_ACTIVE = 1 AND M.PERIOD_TYPE = (SELECT MAX(PERIOD_TYPE) FROM FAS_ADMIN.AKT_MONITORING WHERE IS_ACTIVE = 1 AND SEVEN_ONE_ID = A.ID GROUP BY SEVEN_ONE_ID)
LEFT JOIN AUD_REG.SYSTEM_USER SU ON M.CREATED_BY = SU.USER_ID
  LEFT JOIN (SELECT SEVEN_ONE_ID, EVALUATION_TYPE, SUM(PROCESS_AMOUNT) PROCESS_AMOUNT, MAX(PROCESS_EVALUATION) PROCESS_EVALUATION
    FROM FAS_ADMIN.AKT_EVALUATION 
    WHERE IS_ACTIVE = 1
    GROUP BY SEVEN_ONE_ID, EVALUATION_TYPE) AE ON A.ID = AE.SEVEN_ONE_ID
    AND AE.EVALUATION_TYPE = (SELECT CASE WHEN TD.ID IS NOT NULL AND TD1.ID IS NULL THEN 2
                                WHEN TD.ID IS NOT NULL AND TD1.ID IS NOT NULL THEN 2 
                                WHEN TD.ID IS NULL AND TD1.ID IS NOT NULL THEN 2 
                                ELSE 1 END
                            FROM FAS_ADMIN.FAS_AUDIT FA
                            LEFT JOIN FAS_ADMIN.FAS_AUDIT_TEAM_DATA TD ON FA.ID = TD.FAS_AUDIT_ID AND TD.IS_ACTIVE = 1 AND TD.ROLE_ID = 11
                            LEFT JOIN FAS_ADMIN.FAS_AUDIT_TEAM_DATA TD1 ON FA.ID = TD1.FAS_AUDIT_ID AND TD1.IS_ACTIVE = 1 AND TD1.ROLE_ID = 6
                            WHERE FA.IS_ACTIVE = 1 AND FA.ID = A.FAS_AUDIT_ID)
      ) A ON FA.ID = A.FAS_AUDIT_ID
        ...
        if (req.body.data?.length > 0) {
          req.body.data?.forEach((element) => {
            data.push({
              ...
              P_EXEC_DATE: DateFormat(element.EXEC_DATE),
...
--        NVL(L.SALBAR_ANGILAL, FAS.SALBAR_ANGILAL) SALBAR_ANGILAL         
  ...
  async BM5IU(req, res) {
    try {
      const queryBM5 = `BEGIN AUD_STAT.NEW_BM5_I_U(:P_ID, :P_STAT_AUDIT_ID, :P_AUDIT_ID, :P_AUDIT_TYPE_ID, :P_ID_7_1, :P_IS_TRANSFER, :P_LOG_ID, :P_AUDIT_TYPE_NAME, :P_AUDIT_NAME, :P_AUDIT_CODE, :P_FORM_TYPE_ID, :P_AUDIT_TYPE, :P_AUDIT_ORG_TYPE, :P_AUDIT_ORG_CHECK_NAME, :P_ENT_NAME, :P_ORG_REGISTER_NO, :P_BUDGET_TYPE_ID, :P_BUDGET_SHORT_NAME, :P_CHECK_DEPARTMENT_NAME, :P_AUDIT_DEPARTMENT_NAME, :P_ALD_SHORT_DESC, :P_SOLUTION, :P_SOLUTION_NAME, :P_AMOUNT, :P_IS_ERROR_CONFLICT, :P_IS_ERROR_CONFLICT_NAME, :P_SOLUTION_ERROR, :P_SOLUTION_ERROR_NAME, :P_UR_UGUUJ, :P_UR_UGUUJ_NAME, :P_UR_UGUUJ_TYPE, :P_UR_UGUUJ_TYPE_NAME, :P_AKT_DATE, :P_AKT_NO, :P_SHORT_DESC, :P_COMPLETION_DATE, :P_USER_ID, :P_AUDITOR_NAME, :P_AUDITOR_CODE, :P_MO_DATE, :P_MO_AMOUNT, :P_ACCOUNT_TYPE, :P_ACCOUNT_TYPE_NAME, :P_FULL_NAME, :P_EXEC_DATE, :P_NEXT_AMOUNT, :P_DATE_CNT, :P_SALBAR_ANGILAL, :P_CREATED_BY); END;`;
...
LEFT JOIN AUD_REG.SYSTEM_USER SU ON M.CREATED_BY = SU.USER_ID
  ...
  async BM6IU(req, res) {
    try {
      const queryBM6 = `BEGIN AUD_STAT.NEW_BM6_I_U(:P_ID, :P_STAT_AUDIT_ID, :P_AUDIT_ID, :P_AUDIT_TYPE_ID, :P_ID_7_1, :P_LOG_ID, :P_AUDIT_TYPE_NAME, :P_AUDIT_NAME, :P_AUDIT_CODE, :P_FORM_TYPE_ID, :P_AUDIT_TYPE, :P_AUDIT_ORG_TYPE, :P_AUDIT_ORG_CHECK_NAME, :P_ENT_NAME, :P_ORG_REGISTER_NO, :P_BUDGET_TYPE_ID, :P_BUDGET_SHORT_NAME, :P_CHECK_DEPARTMENT_NAME, :P_AUDIT_DEPARTMENT_NAME, :P_ALD_SHORT_DESC, :P_SOLUTION, :P_SOLUTION_NAME, :P_AMOUNT, :P_IS_ERROR_CONFLICT, :P_IS_ERROR_CONFLICT_NAME, :P_SOLUTION_ERROR, :P_SOLUTION_ERROR_NAME, :P_UR_UGUUJ, :P_UR_UGUUJ_NAME, :P_UR_UGUUJ_TYPE, :P_UR_UGUUJ_TYPE_NAME, :P_AKT_DATE, :P_AKT_NO, :P_SHORT_DESC, :P_COMPLETION_DATE, :P_USER_ID, :P_AUDITOR_NAME, :P_AUDITOR_CODE, :P_MO_DATE, :P_MO_AMOUNT, :P_ACCOUNT_TYPE, :P_ACCOUNT_TYPE_NAME, :P_FULL_NAME, :P_EXEC_DATE, :P_NEXT_AMOUNT, :P_DATE_CNT, :P_SALBAR_ANGILAL, :P_CREATED_BY); END;`;
...
LEFT JOIN AUD_REG.SYSTEM_USER SU ON M.CREATED_BY = SU.USER_ID
  ...
  async BM7IU(req, res) {
    try {
      const queryBM7 = `BEGIN AUD_STAT.NEW_BM7_I_U(:P_ID, :P_STAT_AUDIT_ID, :P_AUDIT_ID, :P_AUDIT_TYPE_ID, :P_ID_7_1, :P_IS_TRANSFER, :P_LOG_ID, 
        :P_AUDIT_TYPE_NAME, :P_AUDIT_NAME, :P_AUDIT_CODE, :P_FORM_TYPE_ID, :P_AUDIT_TYPE, :P_AUDIT_ORG_TYPE, :P_AUDIT_ORG_CHECK_NAME, 
        :P_ENT_NAME, :P_ORG_REGISTER_NO, :P_BUDGET_TYPE_ID, :P_BUDGET_SHORT_NAME, :P_CHECK_DEPARTMENT_NAME, :P_AUDIT_DEPARTMENT_NAME, 
        :P_ALD_SHORT_DESC, :P_SOLUTION, :P_SOLUTION_NAME, :P_AMOUNT, :P_IS_ERROR_CONFLICT, :P_IS_ERROR_CONFLICT_NAME, :P_SOLUTION_ERROR, :P_SOLUTION_ERROR_NAME, :P_UR_UGUUJ, :P_UR_UGUUJ_NAME, :P_UR_UGUUJ_TYPE, :P_UR_UGUUJ_TYPE_NAME, 
        :P_AKT_DATE, :P_AKT_NO, :P_SHORT_DESC, :P_COMPLETION_DATE, :P_USER_ID, :P_AUDITOR_NAME, :P_AUDITOR_CODE, 
        :P_MO_DATE, :P_MO_AMOUNT, :P_ACCOUNT_TYPE, :P_ACCOUNT_TYPE_NAME, :P_FULL_NAME, :P_EXEC_DATE, :P_NEXT_AMOUNT, :P_DATE_CNT, :P_SALBAR_ANGILAL, 
        :P_CREATED_BY); END;`;
...
LEFT JOIN AUD_REG.SYSTEM_USER SU ON M.CREATED_BY = SU.USER_ID
  ...
  async BM8IU(req, res) {
    try {
      const queryBM8 = `BEGIN AUD_STAT.NEW_BM8_I_U(:P_ID, :P_STAT_AUDIT_ID, :P_AUDIT_ID, :P_AUDIT_TYPE_ID, :P_ID_7_1, :P_IS_TRANSFER, :P_TRANSFER_AMOUNT, :P_TRANSFER_DESC, :P_TRANSFER_ORG, :P_TRANSFER_DOC_NO, :P_PERSON_POSITION, :P_LAW_FULL_AMOUNT, :P_LAW_UNDER_AMOUNT, :P_LAW_REJECTED_AMOUNT, :P_LOG_ID, :P_AUDIT_TYPE_NAME, :P_AUDIT_NAME, :P_AUDIT_CODE, :P_FORM_TYPE_ID, :P_AUDIT_TYPE, :P_AUDIT_ORG_TYPE, :P_AUDIT_ORG_CHECK_NAME, :P_ENT_NAME, :P_ORG_REGISTER_NO, :P_BUDGET_TYPE_ID, :P_BUDGET_SHORT_NAME, :P_CHECK_DEPARTMENT_NAME, :P_AUDIT_DEPARTMENT_NAME, :P_ALD_SHORT_DESC, :P_SOLUTION, :P_SOLUTION_NAME, :P_AMOUNT, :P_IS_ERROR_CONFLICT, :P_IS_ERROR_CONFLICT_NAME, :P_SOLUTION_ERROR, :P_SOLUTION_ERROR_NAME, :P_UR_UGUUJ, :P_UR_UGUUJ_NAME, :P_UR_UGUUJ_TYPE, :P_UR_UGUUJ_TYPE_NAME, :P_AKT_DATE, :P_AKT_NO, :P_SHORT_DESC, :P_COMPLETION_DATE, :P_USER_ID, :P_AUDITOR_NAME, :P_AUDITOR_CODE, :P_MO_DATE, :P_MO_AMOUNT, :P_ACCOUNT_TYPE, :P_ACCOUNT_TYPE_NAME, :P_FULL_NAME, :P_EXEC_DATE, :P_NEXT_AMOUNT, :P_DATE_CNT, :P_SALBAR_ANGILAL, :P_CREATED_BY); END;`;
  ...
  async BM9AList(req, res) {
    try {
      ...
      let ListQuery = `SELECT BM9A.ID, BM9A.STAT_AUDIT_ID,
        BM9A.MOVEMENT_TYPE_ID, BM9A.MOVEMENT_SUB_TYPE_ID, BM9A.MOVEMENT_DATE,
        BM9A.TAB_YEAR, BM9A.EDUCATION_TYPE_ID, BM9A.PROFESSION_TYPE_ID,
        HR.*
        ...
        INNER JOIN AUD_REG.SYSTEM_USER SU ON SU.USER_CODE = E.EMP_CODE AND SU.IS_ACTIVE = 1
  ...
  async BM9AIU(req, res) {
    try {
      const queryBM9A = `BEGIN AUD_STAT.NEW_BM9A_I_U(:P_ID, :P_STAT_AUDIT_ID, :P_PERSON_ID, :P_DEPARTMENT_ID, :P_DEPARTMENT_NAME, :P_SUB_DEPARTMENT_ID, :P_SUB_DEPARTMENT_NAME, :P_EMP_CODE, :P_PERSON_LASTNAME, :P_PERSON_FIRSTNAME, :P_PERSON_REGISTER_NO, :P_PERSON_BORNDATE, :P_PERSON_AGE, :P_PERSON_AGE_CLASS, :P_PERSON_GENDER, :P_EMP_SUB_ROLE_NAME, :P_TAB_YEAR, :P_EDUCATION_TYPE_ID, :P_PROFESSION_TYPE_ID, :P_MOVEMENT_TYPE_ID, :P_MOVEMENT_SUB_TYPE_ID, :P_MOVEMENT_DATE, :P_CREATED_BY); END;`;
        ...
        if (req.body.data?.length > 0) {
          req.body.data?.forEach((element) => {
            data.push({
              ...
              P_TAB_YEAR: CheckNullInt(element.TAB_YEAR),
              P_EDUCATION_TYPE_ID: CheckNullInt(element.EDUCATION_TYPE_ID),
              P_PROFESSION_TYPE_ID: CheckNullInt(element.PROFESSION_TYPE_ID),
              P_MOVEMENT_TYPE_ID: CheckNullInt(element.MOVEMENT_TYPE_ID),
              P_MOVEMENT_SUB_TYPE_ID: CheckNullInt(
                element.MOVEMENT_SUB_TYPE_ID
              ),
              P_MOVEMENT_DATE: DateFormat(element.MOVEMENT_DATE),
  ...
  async BM9CList(req, res) {
    try {
      ...
      let ListQuery = `SELECT BM9C.ID, BM1_ID, BM9C.AUDIT_TYPE_ID, RAT.AUDIT_TYPE_NAME, BM9C.DEPARTMENT_ID, D.DEPARTMENT_NAME, BM9C.SUB_DEPARTMENT_ID, SD.SUB_DEPARTMENT_NAME, 
      BM9C.ENT_ID, AE.ENT_NAME, BM9C.AUDIT_CODE, BM9C.REPORT_TYPE_ID, RT.REPORT_TYPE_NAME, 
      ...
      BM9C.REPORTED_ABSTRACT_ID, R1.REPORT_ETYPE_ID REPORTED_ABSTRACT_NAME,
      BM9C.REPORTED_MORE_ID, R2.REPORT_ETYPE_NAME REPORTED_MORE_NAME,
      BM9C.REPORTED_VISUAL_ID, R3.REPORT_ETYPE_NAME REPORTED_VISUAL_NAME, BM9C.IS_ACTIVE
      ...
      LEFT JOIN AUD_STAT.REF_AUDIT_TYPE RAT ON BM9C.AUDIT_TYPE_ID = RAT.AUDIT_TYPE_ID
      LEFT JOIN AUD_STAT.REF_REPORT_TYPE RT ON BM9C.REPORT_TYPE_ID = RT.REPORT_TYPE_ID
      LEFT JOIN AUD_STAT.REF_REPORT_ETYPE R1 ON BM9C.REPORTED_ABSTRACT_ID = R1.REPORT_ETYPE_ID
      LEFT JOIN AUD_STAT.REF_REPORT_ETYPE R2 ON BM9C.REPORTED_MORE_ID = R2.REPORT_ETYPE_ID
      LEFT JOIN AUD_STAT.REF_REPORT_ETYPE R3 ON BM9C.REPORTED_VISUAL_ID = R3.REPORT_ETYPE_ID
      ...
      BM9C.ENT_ID, L.ENT_NAME, L.AUDIT_CODE, BM9C.REPORT_TYPE_ID, NULL,
      BM9C.PAGE_ABSTRACT_COUNT, BM9C.PAGE_MORE_COUNT, BM9C.PAGE_VISUAL_COUNT, 
      BM9C.PRINT_ABSTRACT_COUNT, BM9C.PRINT_MORE_COUNT, BM9C.PRINT_VISUAL_COUNT,
      BM9C.REPORTED_ABSTRACT_ID, NULL, BM9C.REPORTED_MORE_ID, NULL, BM9C.REPORTED_VISUAL_ID, NULL, BM1.IS_ACTIVE
      FROM AUD_STAT.NEW_BM1_DATA BM1
      INNER JOIN AUD_STAT.NEW_BM1_LOG L ON L.BM1_ID = BM1.ID
      INNER JOIN FAS_ADMIN.FAS_AUDIT FA ON BM1.AUDIT_ID = FA.ID
        ...
        if (req.body.data?.length > 0) {
          req.body.data?.forEach((element) => {
            data.push({
              P_ID: element.ID != null ? parseInt(element.ID) : null,
              P_AUDIT_TYPE_ID: CheckNullInt(element.AUDIT_TYPE_ID),
              P_REPORT_TYPE_ID: CheckNullInt(element.REPORT_TYPE_ID),
              P_DEPARTMENT_ID: CheckNullInt(element.DEPARTMENT_ID),
              P_SUB_DEPARTMENT_ID: CheckNullInt(element.SUB_DEPARTMENT_ID),
              P_BM1_ID: CheckNullInt(element.BM1_ID),
  ...
  async BM11List(req, res) {
    try {
      ...
      BM11.IS_FULFILLMENT, BM11.RECOMMENDATION_TYPE_ID, RT.RECOMMENDATION_TYPE_NAME, BM11.DECIDED_COUNT, BM11.UNIMPLEMENTED_REASON,BM11.CREATED_BY
